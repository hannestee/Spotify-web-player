	...
	function onGetTracks(data) {
		...
		for (var i in tracksAvailable){
			...
			if (tracksAvailable[i].preview_url) {
				...
				tracksAvailable[i].artists[0].name+
				"</p>"+"</div>"+"<p class='playListAdd' style='display:inline-block;font-size:24px;margin-left:6em;text-overflow:ellipsis;white-space:pre;' onclick='addToPlaylist(\"" +
				tracksAvailable[i].preview_url + "\",\"" +
				tracksAvailable[i].name + "\",\"" +
				tracksAvailable[i].artists[0].name + "\",\"" +
		...
		document.getElementById('footer').setAttribute("style", "position:absolute;right:0;left:0;bottom:0;");
